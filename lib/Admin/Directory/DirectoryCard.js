...
import { updateUserProfile, deleteUserProfile } from '../../src/GlobalFunctions/API/APICalls.js';
...
function DirectoryCard (props) {
    ...
    const updateCard = (mail, first, last, value, pnum, pnouns) => {
        email = mail.toLowerCase(); fname = first; lname = last; role = value; phone = pnum; pronouns = pnouns;
        setMail(mail.toLowerCase()); setFirst(first); setLast(last); setValue(value), setPNum(pnum), setPnouns(pnouns);
    }
    ...
    return( <View><Text>No user in direcotry.</Text></View>);
}

...
function RegistrationRequestCard (props) {
    ...
    const updateCard = (m, f, l, v, ph, pn) => {
        email = m; fname = f; lname = l; role = v; phone = ph; pronouns = pn;
        setMail(m); setFirst(f); setLast(l); setValue(v), setPNum(ph); setPnouns(pn);
    }
    ...
    const addToProfiles = async () => {
        ...
        Alert.alert("Add this user profile?", `Would you like to add user: ${fname} ${lname}'s to the Project Friendship database?`,
        [
            {
                text: 'Cancel',
                style: 'cancel'
            ...
            { 
                text: "Add User", 
                onPress: async () => {
                    await createUserProfile(email, fname, lname, role, phone, pronouns);
                    await deleteRegistrationRequest(email);

...
function Profile ({ route, navigation }) {
  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    if (role === "mentor") {
      // can add a child only
    } else if (role === "parent"){ 
      var user = await handlePress('sessions', 'PUT', { // adds into votes. 
        headers: {"Content-type": formContentType}, body: `sid=${sid}&`});
        // can add a child OR add a mentor
    }
      // var name = 'fefoiehof';
  ...
  const fetchUserRelationships = async (email) => {
    ...
    for(let i = 0; i < relations.length; i++) {
      const {fname, lname} = await handlePress('child', 'PUT', {
        headers: {"Content-type": formContentType}, body: `cid=${relations[i].cid}&`});

      // find mentor 
      let {email} = await handlePress('child/childrs/findMentorEmail', 'PUT', {
        headers: {"Content-type": formContentType}, body: `cid=${relations[i].cid}&`});

      
      var hasMentor = 0;

      var mfname = "None appointed";
      if (email) {
          console.log("Child has a mentor!"); 
          hasMentor = 1;
          // query for mentor name 
          var { mfname, mlname, mphone, memail } = await handlePress('users', 'PUT', {
            headers: {"Content-type": formContentType}, body: `email=${email}&`});
      } else { console.log("child does not have a mentor!"); }
          ...
          },
          {
              ...
              onPress: async() => { 
                  ...
                  await handlePress('child', 'POST', {
                      ...
                      body: `fname=${childFname}&lname=${childLname}&dob=${childDOB}&school=${childSchool}&pronouns=${childPronouns}`});
    ...
    <View style={styles.container}>
          ...
          <View style={styles.modalView}>
            ...
            <ScrollView style={{borderRadius: 7, height: 250, width: 300, paddingVertical: 10}}>
              ...
              { rs.map((item, index) => { //<Text key={index}>{item.person}, {item.role}</Text>
                      return  <RelationshipCard key={index} child={item} 
                      modalVisible={setModalVisibleRelations}/>
              })} 
            {/* <View style={{backgroundColor: "white", padding: 10,}}>
              <Text style={{fontWeight: 'bold'}}>Areeba</Text>
            </View> */}
            </ScrollView>
            <View style={{flex: 1, flexDirection: "row"}}>
                ...
                {!isParent && <Button title='Generate my QR code'
                  onPress={()=> { 
                    setModalVisibleRelations(false); 
                    generateQRCode(sid)}}/>}
              ...
              <View style={{backgroundColor: 'lightpink', height: 200}}></View>
            </ScrollView>  */}
      ...
      <Modal
        ...
        onRequestClose={() => {
          ...
          <View style={[styles.modalView, {marginTop: 230}]}>

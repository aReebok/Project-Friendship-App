...
function Profile ({ route, navigation }) {
  ...
  const fetchUserRelationships = async (email) => {
    var relations = await handlePress('childrs', 'PUT', {
      headers: {"Content-type": formContentType}, body: `email=${email}&`});
    var arr = [];
    console.log(relations);
    for(let i = 0; i < relations.length; i++) {
      const {fname, lname, notes, pronouns, dob} = await handlePress('child', 'PUT', {
        headers: {"Content-type": formContentType}, body: `cid=${relations[i].cid}&`});

      // find mentor 
      let mentor = await handlePress('childrs/findMentorEmail', 'PUT', {
        headers: {"Content-type": formContentType}, body: `cid=${relations[i].cid}&`});

...
function Profile ({ route, navigation }) {
  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    if (child) {
      ...
      if (role === "mentor") {
        try {
          ...
          if(rs.length) {
            console.log("RS:"+rs)
            Alert.alert("You're already matched with a mentee.");
          } else {
    ...
    // if (user.email) { alert(`Would you like to friend ${user.email}`);
    // } else {
    //    Alert.alert('Failed Scan', 
  ...
  const fetchUserRelationships = async (email) => {
    ...
    for(let i = 0; i < relations.length; i++) {
      ...
      if (email) {
          ...
          const { fname, lname, phone } = await handlePress('users', 'PUT', {
            headers: {"Content-type": formContentType}, body: `email=${email}&`});
          // console.log(fname);
          var mentorInfo = {
            first: fname,

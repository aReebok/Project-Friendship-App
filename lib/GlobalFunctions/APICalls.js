...
export const createUserProfile = async (email, first, last, role, phone) => {
    return await handlePress('users', 'POST', {  
        headers: {"Content-type": formContentType}, 
        body: `email=${email}&fname=${first}&lname=${last}&role=${role}&phone=${phone}`});
}

export const createRegistrationRequest = async (first, last, email, phone, role) => {
    return await handlePress('register', 'POST', { // adds into votes. 
        headers: {"Content-type": formContentType}, 
        body: `fname=${first}&lname=${last}&email=${email.toLowerCase()}&phone=${phone}&role=${role}`});
}
 ...
 */
 ...
 export const updateUserProfile = async (e, mail, first, last, value, pnum) => {
    await deleteUserProfile(e);
    return await createUserProfile(mail, first, last, value, pnum);
}

export const updateRegistrationRequest = async (e, mail, first, last, value, pnum) => {
    await deleteRegistrationRequest(e);
    await createRegistrationRequest(first, last, mail, pnum, value);
}
...
const  RegisterScreen = (props) => {  
    ...
    const handleRegister = async () => {
      ...
      if (checked && fname && lname && email && phone && value) {
        ...
        var request = await handlePress('register/request', 'PUT', { // adds into votes. 
          headers: {"Content-type": formContentType}, body: `email=${email}`});
        ...
        await handlePress('register/request', 'POST', { // adds into votes. 
          headers: {"Content-type": formContentType}, 
          body: `fname=${fname.toLowerCase()}&lname=${lname}&email=${email}&phone=${phone}&role=${value}`});

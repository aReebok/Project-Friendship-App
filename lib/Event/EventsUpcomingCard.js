                ...
                <View style={styles.notifCard_event}>
                    <View style={{flex: 2.5, flexDirection:'row', }}>
                        ...
                        <View style={{flex:2}}>
                            ...
                            {<Text style={styles.subtitle}>Created by {event.authorfname} {event.authorlname[0]}. </Text>}
                            <Text style={[styles.subtitle, {fontWeight: '500'}]}>Description: <Text style={{fontWeight: '400'}}>{event.descrip}</Text></Text>
                            {<Text style={styles.subtitle}>Approved by {event.approverfname} {event.approverlname[0]}. </Text>}
                            {/* <Text style={styles.subtitle}>When: <Text style={{fontWeight: '500'}}>{date.toLocaleDateString('en-US', optionsDate)} at {date.toLocaleTimeString('en-US', optionsTime)}</Text> </Text> */}
                            { Platform.OS === 'ios' ?<Text style={styles.subtitle}>When: <Text style={{fontWeight: '500'}}>{date.toLocaleDateString('en-US', optionsDate)} at {date.toLocaleTimeString('en-US', optionsTime)}</Text></Text> :
                            <Text style={styles.subtitle}>When: <Text style={{fontWeight: 'bold'}}>{Intl.DateTimeFormat('en-US', optionsDate).format(date)} at {Intl.DateTimeFormat('en-US', optionsTime).format(date)}</Text></Text>}
                        </View>
                    </View>

...
function EventForm (props) {
    ...
    const {sid, date, setDate, event,
        eventEditingFormModalVisible, setEventEditingFormModalVisible, 
        email } = props;

    const [value, setValue] = useState(event.eventdate);
    ...
    const sendEvent = async () => {
        ...
        await handlePress('events', 'POST', { // posts new updated time event 
            headers: {"Content-type": formContentType},
            body: `author=${email}&cid=${value}&title=${event.title}&stat=${'edited'}&descrip=${event.descrip}&eventDate=${date}&eventCreated=${new Date(Date.now())}&eventLocation=${event.eventlocation}`});  
        // // reset values
    ...
    return (
        <Modal
            ...
            <View style={styles.modalView}>
                ...
                <Text style={styles.pickedDate}>{dateFormat.toLocaleString('en-US', options)}</Text>

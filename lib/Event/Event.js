...
const Event = (props) => {
    ...
    const updatePendingEvents = async (cid, child) => {
        let childEvents = await APICalls.getPendingEventsByChildID(cid);
    ...
    const updateUpcomingEvents = async (cid, child) => {
        let childEvents = await APICalls.getApprovedEventsByChildID(cid);
    ...
    const setDropDownItems = async (email, role, relations) => {
        ...
        for (let i = 0; i < relations.length; i++) {     
            let { cid } = relations[i];
            var child = await APICalls.getChildProfileByChildID(cid);

            updatePendingEvents(cid, child);
            updateUpcomingEvents(cid, child);
...
return ( <View style={styles.container}>
    <EventForm modalVisible={modalVisible} setModalVisible={setModalVisible} 
        ...
        setItems={setItems} items={items} date={date}
        setDate={setDate}  sid={sid}/>

        ...
        if (role === "parent") {
            ...
            for(let i = 0; i < relations.length; i++) {
                ...
                try {
                    ...
                    let user = await handlePress('users', 'PUT', {
                        headers: {"Content-type": formContentType}, body: `email=${email}&`});
                    pushRSToArray(arr, `${user.fname} (${child.fname}'s mentor)`, 123);
                } catch (err){ console.log("Error: Child doesn't have mentor")}
            ...
            for(let i = 0; i < relations.length; i++) {
                ...
                try {
                    email = mentor[0].email;
                    console.log(email);
                    pushRSToArray(arr, `${child.fname}`, 123);
                } catch (err){ console.log("Error: Child doesn't have mentor")}
                
            }
        } else {
            return console.log("Error: user is neither a parent nor a mentor");
        }

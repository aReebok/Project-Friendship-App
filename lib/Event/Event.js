...
import { Dimensions, Button, View, StyleSheet, ScrollView, Modal } from 'react-native';
...
const Event = (props) => {
    ...
    const fetchRelations = async () => {
        var { email } = await handlePress('session/get', 'PUT', {
            headers: {"Content-type": formContentType}, body: `sid=${sid}&`});
        var relations = await handlePress('relationship/get', 'PUT', {
            headers: {"Content-type": formContentType}, body: `email=${email}&`});
        var arr = [];
        for(let i = 0; i < relations.length; i++) {
            if (relations[i].person1 === email) {
                var { fname } = await handlePress('users/get', 'PUT', {
          ...
          <View style={styles.modalView}>
            ...
            <DropDownPicker
                ...
                <View style={styles.btnContainer}>
                    <Button title="Pick Time" color="purple" onPress={showPicker} />

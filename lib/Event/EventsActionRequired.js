...
function EventsRequests(props) {
    const { email, role, eventsArray } = props;
            ...
            {eventsArray.map((item, index)=>{
                ...
                if (item.stat === 'pending') {
                    if (item.author === email) {
                        ...
                        // console.log(`${item.eid}: ${item.stat} by ${item.author} (me).`)
                    } else {
                        bannerStyle.title = "New Event Request";
                        bannerStyle.color = logo.red;
                        // console.log(`${item.eid}: ${item.stat} by ${item.author}.`)
                    }
                ...
                return <EventsActionRequiredCard key={index} 
                    email={email} event={item} bannerStyle={bannerStyle}/>
            })}

...
function EventsRequests(props) {
    const { email, role, eventsArray, arrlen } = props;
    ...
    return (<View style={styles.card}> 
        <View style={{flexDirection: 'row'}}> 
            ...
            : <View><Icon name='check' size={30} color={'green'}/></View>}
            ...
            {eventsArray.map((item, index) => {
                ...
                if (item.stat === 'pending') {
                    ...
                    if (item.author === email) {
                        ...
                        // const user = await getUserProfile(item.author);
                        bannerStyle.authorStr = `${item.author}`;
                    }

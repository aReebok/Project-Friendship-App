...
function EventForm (props) {
    ...
    const sendEvent = async () => {
        ...
        var user = await APICalls.getUserProfile(email);

        if(user.role === 'parent') {
            // is parent, recipient is mentor
            var mentor = await APICalls.getChildMentorEmailByChildID(cid);
        ...
        await APICalls.createEvent(email, value, eventTitle, 'pending',
            eventDesc, date, eventCreated, eventLocation);

        const event = await APICalls.getEventsByEmail(email);

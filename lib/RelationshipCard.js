...
function RelationshipCard (props) {
    ...
    const { person, role, hasMentor, cid, mentorInfo, parentsInfo } = props.child;
                    ...
                    <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                        <View style={{flex:3}}>
                            ...
                            <View style={{flexDirection: 'row'}}>
                                ...
                                <Text> Notes: My child has nut alergies. </Text>
                            </View>
                        </View>
    ...
    if (!isParent) {
        ...
        return(<View style={{backgroundColor: '#f0f0f0', padding: 10, marginBottom: 5, borderRadius: 5}}>
            <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                <View>
                    <Text style={{fontWeight: '500'}}>{person}, <Text style={{fontWeight:'300'}}> my mentee.</Text></Text>
                    <Text style={{fontWeight: '500'}}>Notes: <Text style={{fontWeight:'300'}}> Henry has nut alergies.</Text></Text>
                </View>
            </View>
            <View style = {styles.lineStyle} />
            <Text style={{paddingTop: 5, fontWeight: '500'}}>Parents Contacts:</Text>
            { parentsInfo.map((item, index) => {
                return <View style={{backgroundColor: '#f0f0f0'}}>
                            <Text style={{fontWeight: '500'}}>{item.first} {item.last[0]}. <Text style={{fontWeight:'300'}}>(pronouns)</Text></Text>
                            <View style={{flexDirection: 'row'}}>
                                <Text>Phone Number: </Text>
